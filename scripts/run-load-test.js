...
seq
    ...
    .then(() => console.log(`${events.length} events with ${quotas.length} quotas and ${questions.length} questions created.`))
    .then(() => seq.close())
        ...
        setTimeout(function timer() {
            axios.post(apiUrl, { quotaId: 1 })
                .then(function (response) {
                    ...
                    axios.patch(apiUrl + response.data.id, postData)
                        .then(function (response) {
                            console.log("Success")
                        })
